...
TEST_CASE("fiber.c", "[fiber]") {
    ...
    SECTION("fiber_new") {
        SECTION("allocate a new fiber") {
            ...
            fiber_t * fiber = fiber_new(test_fiber_name, stack_size, test_fiber_new_empty, &user_data);
        ...
        SECTION("fail to allocate a new fiber without stack") {
            fiber_t *fiber = fiber_new(test_fiber_name, 0, NULL, NULL);
            REQUIRE(fiber == NULL);
        }
    }
    ...
    SECTION("fiber_free") {
        ...
        SECTION("allocate a new fiber") {
            fiber_t *fiber = fiber_new(test_fiber_name, stack_size, NULL, NULL);
            fiber_free(fiber);
        }
    }
