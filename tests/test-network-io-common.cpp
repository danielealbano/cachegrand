...
uint16_t test_network_io_common_bind_search_free_port_ipv6(
        uint16_t start_port) {
    uint16_t port;
    int val = 1;
...
TEST_CASE("network/io/network_io_common", "[network][network_io][network_io_common]") {
    ...
    uint16_t socket_port_free_ipv4 =
            test_network_io_common_bind_search_free_port_ipv4(9999);
    uint16_t socket_port_free_ipv6 =
            test_network_io_common_bind_search_free_port_ipv6(9999);
    ...
    SECTION("network_io_common_socket_tcp6_new") {
        int fd = network_io_common_socket_tcp6_new();
        REQUIRE(fd > 0);
        close(fd);
    }
