...
static void memory_allocation_fast_fixed_memory_allocator_hugepages_warmup_cache(benchmark::State& state) {
...
static void memory_allocation_fast_fixed_memory_allocator_only_alloc(benchmark::State& state) {
    ...
    for (auto _ : state) {
        for(long int i = 0; i < objects_count; i++) {
            benchmark::DoNotOptimize((memptrs[i] = fast_fixed_memory_allocator_mem_alloc(object_size)));
        }
    }

    for(long int i = 0; i < objects_count; i++) {
        fast_fixed_memory_allocator_mem_free(memptrs[i]);
    }
}

static void memory_allocation_fast_fixed_memory_allocator_alloc_and_free(benchmark::State& state) {
    size_t object_size = state.range(0);
    uint32_t objects_count = state.range(1);
...
static void memory_allocation_fast_fixed_memory_allocator_fragment_memory(benchmark::State& state) {
    size_t object_size = state.range(0);
    uint32_t objects_count = state.range(1);
...
static void BenchArguments(benchmark::internal::Benchmark* b) {
    b
            ->ArgsProduct({
                { FAST_FIXED_MEMORY_ALLOCATOR_OBJECT_SIZE_16, FAST_FIXED_MEMORY_ALLOCATOR_OBJECT_SIZE_32, FAST_FIXED_MEMORY_ALLOCATOR_OBJECT_SIZE_64, FAST_FIXED_MEMORY_ALLOCATOR_OBJECT_SIZE_128, FAST_FIXED_MEMORY_ALLOCATOR_OBJECT_SIZE_256,
                  FAST_FIXED_MEMORY_ALLOCATOR_OBJECT_SIZE_512, FAST_FIXED_MEMORY_ALLOCATOR_OBJECT_SIZE_1024, FAST_FIXED_MEMORY_ALLOCATOR_OBJECT_SIZE_2048, FAST_FIXED_MEMORY_ALLOCATOR_OBJECT_SIZE_4096,
                  FAST_FIXED_MEMORY_ALLOCATOR_OBJECT_SIZE_8192, FAST_FIXED_MEMORY_ALLOCATOR_OBJECT_SIZE_16384, FAST_FIXED_MEMORY_ALLOCATOR_OBJECT_SIZE_32768, FAST_FIXED_MEMORY_ALLOCATOR_OBJECT_SIZE_65536 },
...
BENCHMARK(memory_allocation_fast_fixed_memory_allocator_hugepages_warmup_cache)
        ->Iterations(1)
        ->Repetitions(1);

BENCHMARK(memory_allocation_fast_fixed_memory_allocator_only_alloc)
        ->Apply(BenchArguments);
BENCHMARK(memory_allocation_fast_fixed_memory_allocator_alloc_and_free)
        ->Apply(BenchArguments);
BENCHMARK(memory_allocation_fast_fixed_memory_allocator_fragment_memory)
        ->Apply(BenchArguments);
