...
bool network_channel_init(
    ...
    if (channel->type == NETWORK_CHANNEL_TYPE_CLIENT) {
        ...
        channel->buffers.send.data = fast_fixed_memory_allocator_mem_alloc(channel->buffers.send.length);
    }
...
void network_channel_cleanup(
        network_channel_t *channel) {
    if (channel->type == NETWORK_CHANNEL_TYPE_CLIENT) {
        fast_fixed_memory_allocator_mem_free(channel->buffers.send.data);
