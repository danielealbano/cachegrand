...
void module_redis_accept(
    ...
    // Ensure that the command context is always freed if the connection fail and data are allocated
    ...
    // Ensure that the connection context is always freed if the connection fail and data are allocated
    module_redis_connection_context_reset(&connection_context);

    // Cleanup the client context
    module_redis_connection_context_cleanup(&connection_context);
}
...
bool module_redis_process_data(
    ...
    do {
        ...
        do {
            ...
            for (uint8_t op_index = 0; op_index < (uint8_t)ops_found; op_index++) {
                ...
                if (op->type == PROTOCOL_REDIS_READER_OP_TYPE_ARGUMENT_DATA && op->data.argument.index == 0) {
                    ...
                    if ((connection_context->command.context = module_redis_command_alloc_context(
                            connection_context->command.info)) == NULL) {
                ...
                if (is_argument_op && op->data.argument.index > 0) {
                    ...
                    if (connection_context->command.info->argument_require_stream_funcptr) {
                        require_stream = connection_context->command.info->argument_require_stream_funcptr(
                    ...
                    if (op->type == PROTOCOL_REDIS_READER_OP_TYPE_ARGUMENT_BEGIN && require_stream) {
                        if (connection_context->command.info->argument_stream_begin_funcptr) {
                            if (!connection_context->command.info->argument_stream_begin_funcptr(
                                    connection_context->network_channel,
                                    db,
                                    connection_context,
                        ...
                        if (connection_context->command.info->argument_stream_data_funcptr) {
                        ...
                        if (connection_context->command.info->argument_stream_end_funcptr) {
                            if (!connection_context->command.info->argument_stream_end_funcptr(
                                    connection_context->network_channel,
                                    db,
                                    connection_context,
                        ...
                        if (connection_context->command.info->argument_full_funcptr) {
                    ...
                    if (!connection_context->command.info->command_end_funcptr(
                            connection_context->network_channel,
                            db,
                            connection_context,
                            reader_context)) {
        ...
        while (
                ...
                reader_context->error != PROTOCOL_REDIS_READER_ERROR_OK);
