...
#include "module_redis_autogenerated_commands_key_specs.h"
...
void module_redis_reset_context(
    ...
    connection_context->command = MODULE_REDIS_COMMAND_NOP;
    connection_context->command_info = NULL;
    connection_context->command_context  = NULL;
    connection_context->skip_command = false;
    connection_context->command_length = 0;
...
bool module_redis_process_data(
    ...
    protocol_redis_reader_context_t *reader_context = &connection_context->reader_context;
    ...
    do {
        ...
        do {
            ...
            for (uint8_t op_index = 0; op_index < (uint8_t)ops_found; op_index++) {
                ...
                connection_context->command_length += op->data_read_len;

                if (connection_context->command_length > channel->module_config->redis->max_command_length) {
                    send_buffer = send_buffer_start = network_send_buffer_acquire_slice(channel, slice_length);
                    if (send_buffer_start == NULL) {
                        LOG_E(TAG, "Unable to acquire send buffer slice!");
                    }
                ...
                if (connection_context->skip_command) {
                ...
                if (op->type == PROTOCOL_REDIS_READER_OP_TYPE_ARGUMENT_DATA && op->data.argument.index == 0) {
                    ...
                    connection_context->command = MODULE_REDIS_COMMAND_UNKNOWN;
                    ...
                    for (uint32_t command_index = 0; command_index < command_infos_map_count; command_index++) {
                        ...
                        if (command_length == command_info->length &&
                            ...
                            LOG_D(
                                    ...
                                    command_info->string);
                            connection_context->command = command_info->command;
                            connection_context->command_info = command_info;
                    ...
                    if (connection_context->command == MODULE_REDIS_COMMAND_UNKNOWN) {
                        // Command unknown, mark it to be skipped
                        connection_context->skip_command = true;
                        ...
                        *(command_data + command_length) = 0;
                        ...
                        if (connection_context->command_info->required_arguments_count >
                            reader_context->arguments.count - 1) {
                            connection_context->skip_command = true;
                            continue;
                        }
                        ...
                        if ((connection_context->command_context = module_redis_command_alloc_context(
                                connection_context->command_info)) == NULL) {
                ...
                if (is_argument_op && op->data.argument.index > 0) {
                    ...
                    if (connection_context->command_info->argument_require_stream_funcptr) {
                        require_stream = connection_context->command_info->argument_require_stream_funcptr(
                                channel,
                                db,
                                connection_context,
                                reader_context,
                    ...
                    if (op->type == PROTOCOL_REDIS_READER_OP_TYPE_ARGUMENT_BEGIN && require_stream) {
                        if (connection_context->command_info->argument_stream_begin_funcptr) {
                            if (!connection_context->command_info->argument_stream_begin_funcptr(
                                    channel,
                                    db,
                                    connection_context,
                        ...
                        if (connection_context->command_info->argument_stream_data_funcptr) {
                        ...
                        if (connection_context->command_info->argument_stream_end_funcptr) {
                            if (!connection_context->command_info->argument_stream_end_funcptr(
                                    channel,
                                    db,
                                    connection_context,
                        ...
                        if (connection_context->command_info->argument_full_funcptr) {
                    ...
                    if (!connection_context->command_info->command_end_funcptr(
                            channel,
                            db,
                            connection_context,
                            reader_context)) {
        ...
        if (reader_context->error != PROTOCOL_REDIS_READER_ERROR_OK) {
            ...
            if (connection_context->command == MODULE_REDIS_COMMAND_UNKNOWN) {
                ...
                module_redis_reset_context(connection_context);
                break;
            }

            assert(connection_context->command_info != NULL);
            ...
            if (connection_context->command_info->required_arguments_count >
                ...
                send_buffer_start = protocol_redis_writer_write_simple_error_printf(
                        ...
                        connection_context->command_info->string);
                network_send_buffer_release_slice(
                        channel,
                        send_buffer_start ? send_buffer_start - send_buffer : 0);
            ...
            // If the command is all parsed the command data in module command context can be freed
            if (connection_context->command_context) {
                module_redis_command_free_context(
                        connection_context->command_info,
                        connection_context->command_context);
            }
    ...
    if (unlikely(!return_result)) {
        ...
        if (connection_context->command_info != NULL && connection_context->command_context != NULL) {
            module_redis_command_free_context(
                    connection_context->command_info,
                    connection_context->command_context);
        }
