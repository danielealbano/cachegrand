...
void module_redis_connection_context_init(
    ...
    connection_context->read_buffer.data = (char *)fast_fixed_memory_allocator_mem_alloc_zero(NETWORK_CHANNEL_RECV_BUFFER_SIZE);
...
void module_redis_connection_context_cleanup(
    ...
    if (connection_context->client_name) {
        fast_fixed_memory_allocator_mem_free(connection_context->client_name);
    }
    fast_fixed_memory_allocator_mem_free(connection_context->read_buffer.data);
}
...
void module_redis_connection_context_reset(
    ...
    if (connection_context->error.message != NULL) {
        fast_fixed_memory_allocator_mem_free(connection_context->error.message);
...
bool module_redis_connection_error_message_vprintf_internal(
    ...
    char *error_message_with_args = fast_fixed_memory_allocator_mem_alloc(error_message_with_args_length + 1);
