...
worker_module_context_t *worker_module_contexts_initialize(
    ...
    worker_module_context =
            fast_fixed_memory_allocator_mem_alloc_zero(sizeof(worker_module_context_t) * config->modules_count);
    ...
    for(int module_index = 0; module_index < config->modules_count; module_index++) {
        ...
        if (cipher_suites) {
            fast_fixed_memory_allocator_mem_free(cipher_suites);
        }
    ...
    if (!result_ret && worker_module_context) {
        ...
        fast_fixed_memory_allocator_mem_free(worker_module_context);
