...
bool worker_storage_read(
        ...
        size_t buffer_len,
        off_t offset) {
    storage_io_common_iovec_t iov[0];

    iov[0].iov_base = buffer;
    iov[0].iov_len = buffer_len;

    return worker_storage_readv(channel, iov, 1, buffer_len, offset);
}
...
bool worker_storage_write(storage_channel_t *channel,
                          char *buffer,
                          size_t buffer_len,
                          off_t offset) {
    storage_io_common_iovec_t iov[0];
    ...
    return worker_storage_writev(channel, iov, 1, buffer_len, offset);
}
