...
storage_db_t* storage_db_new(
    ...
    for(uint32_t worker_index = 0; worker_index < workers_count; worker_index++) {
        ring_bounded_spsc_t *deleted_entry_index_ring_buffer =
                ring_bounded_spsc_init(STORAGE_DB_WORKER_ENTRY_INDEX_RING_BUFFER_SIZE);
    ...
    if (workers) {
        for(uint32_t worker_index = 0; worker_index < workers_count; worker_index++) {
            if (workers[worker_index].deleted_entry_index_ring_buffer) {
                ring_bounded_spsc_free(workers[worker_index].deleted_entry_index_ring_buffer);
            }
...
void storage_db_deleted_entry_ring_buffer_per_worker_free(
    ...
    ring_bounded_spsc_t *rb = db->workers[worker_index].deleted_entry_index_ring_buffer;

    if (!rb) {
        return;
    }

    while((entry_index = ring_bounded_spsc_dequeue(rb)) != NULL) {
...
storage_db_entry_index_t *storage_db_entry_index_ring_buffer_new(
    ...
    ring_bounded_spsc_t *scb = storage_db_worker_deleted_entry_index_ring_buffer(db);

    if (ring_bounded_spsc_is_full(scb)) {
        entry_index = ring_bounded_spsc_dequeue(scb);
...
void storage_db_entry_index_ring_buffer_free(
    ...
    if (ring_bounded_spsc_is_full(scb)) {
        storage_db_entry_index_t *entry_index_to_free = ring_bounded_spsc_dequeue(scb);
        storage_db_entry_index_free(db, entry_index_to_free);
    }

    ring_bounded_spsc_enqueue(scb, entry_index);
}
