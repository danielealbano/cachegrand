...
epoch_operation_queue_t *epoch_operation_queue_init() {
    ...
    epoch_operation_queue->ring = ring_bounded_queue_spsc_uint128_init(EPOCH_OPERATION_QUEUE_RING_SIZE);
...
void epoch_operation_queue_free(
    ...
    ring_bounded_queue_spsc_uint128_free(epoch_operation_queue->ring);
...
epoch_operation_queue_operation_t *epoch_operation_queue_enqueue(
    ...
    epoch_operation_queue_operation_t *operation_ptr =
            (epoch_operation_queue_operation_t *)ring_bounded_queue_spsc_uint128_enqueue_ptr(
...
void epoch_operation_queue_mark_completed(
    ...
    epoch_operation_queue_operation->data.end_epoch = intrinsics_tsc();
}
...
uint64_t epoch_operation_queue_get_latest_epoch(
    ...
    do {
        operation._packed = ring_bounded_queue_spsc_uint128_peek(epoch_operation_queue->ring, &found);

        if (!found) {
            break;
        }

        if (operation.data.end_epoch == 0) {
            break;
        }

        ring_bounded_queue_spsc_uint128_dequeue(epoch_operation_queue->ring, NULL);
        epoch_operation_queue->latest_epoch = operation.data.end_epoch;
    } while(true);
